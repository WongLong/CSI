...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  Class.prototype.config = {
    limit: 10 //每页显示的数量
    ,loading: false //请求数据时，是否显示loading //2018-03-19 xugy 插件原来默认是true,因为和平台loading冲突，故修改layUI表格加载时默认为不加载loading
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    ...
    ,render = function(){ //后续性能提升的重点
      ...
      try{
    	  $('[title]').tooltip({container: document.body});
      }catch(e){}
      
      layui.event.call(this, MOD_NAME, 'afterRender('+ filter +')', {
          contaier: that,
          data: data
      });
    };
    ...
    if(options.page){
      options.page = $.extend({
        ...
        ,groups: 3
        ,layout: ['page', 'prev', 'next', 'skip', 'count', 'limit']
  ...
  Class.prototype.fullSize = function(){
    ...
    if(layHeaderHeight<=0)layHeaderHeight = options.cols && options.cols.length ? options.cols.length*38 : 38; //yangzh:对于隐藏tab页里加载table,导致header的高度取不到
  ...
  //滚动条补丁
  Class.prototype.scrollPatch = function(lastDragField){
    var that = this
    ,options = that.config
    ...
    ,layMainTableWidth =layMainTable.outerWidth()
    ,outWidth = layMainTableWidth - that.layMain.width(); //表格内容器的超出宽度
    if(layMainTableWidth==0){
    	return;
    }
    var field = '';
    var hasField =false;
    ...
    if(outWidth<5){
    	that.getCssRule(field, function(item){
	        ...
	        var styleWidth = parseFloat(width) - getScrollWidth - outWidth;
	        if(styleWidth<150){
	        	styleWidth =150;
	        }
	        item.style.width = styleWidth + 'px';
    	...
    	if(lastDragField!=field){
	    	that.getCssRule(field, function(item){
		        var width = item.style.width || th.outerWidth();
		        var floatWidth = parseFloat(width);
		        var styleWidth =floatWidth -outWidth;
		        if(styleWidth<150){
		        	styleWidth=150;
		        }
		        item.style.width = styleWidth + 'px';
...
//    }
  ...
  Class.prototype.events = function(){
    ...
    that.layBody.on('click', 'td', function(){
      ...
      if(typeof that.notBind=='undefined'){
	      $("body").bind("mousedown",function(event) {
	    	  if(that.tipsIndex!=null){
  				  if (!(event.target.className === "layui-table-tips-main")) {
  						layer.close(that.tipsIndex);
  						that.tipsIndex=null;
  				  }
  			   }
  ...
  table.init = function(filter, settings){
    ...
    elemTable.each(function(){
      ...
      /**yangzh:当列表创建过了,且div还没有被销毁，则不再创建table*/
      if(filter ==undefined){/**只有当filter为空时阻止重复创建*/
	      if(tableData.id !=  undefined && thisTable.config[tableData.id] !=undefined ){
	    	  var renderClass = tableClassCache[tableData.id];
	    	  if(renderClass!=null){
	    		  var index = renderClass.index;
	    		  var ele = $("#LAY-table-"+index);
	    		  if(ele.length>0){
	    			  return;
	    		  }
	    	  }
  ...
  table.reload = function(id, options){
    ...
    if(options.staticWhere!=undefined){
    	config.staticWhere = options.staticWhere;
    }
    options = options || {};
    config.where = config.staticWhere||{};
  ...
  table.render = function(options){
	  ...
	  if(options.id){
		  ...
		  if(typeof options.elem=='string'){
			  id = options.elem.replace('#','').replace('.','')
		  }else{
			  id = $(options.elem).attr('id');
		  }
	  }
	  var inst = new Class(options);
	  if(tableClassCache[id]!=undefined){
		  inst.autoColNums =tableClassCache[id].autoColNums;
	  }else{
		  tableClassCache[id] = inst;
	  }
